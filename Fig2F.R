# animal diet CAP beased on bray_curtis
...
all_bDAT <- read.delim("otu_rare_coi.txt", row.names= 1,  header=T, sep="\t")
design<-design[colnames(all_bDAT),]
bDAT <- all_bDAT[, rownames(design) ]
bTAX <- read.table( "coi_otu_tax.txt" , row.names=1, sep="\t", header=T)
bTAX <- bTAX[,c(1:7)]
...
pdf("Field_CAP_season_coi.pdf" ,width=5, height=3,)
