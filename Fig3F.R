...
df1 =  read.table("otu_rare_rbcl.txt", header=T, row.names= 1, sep="\t") 
...
grass_food_micro <- ggplot(plot) +
  geom_point(size = 4, alpha=0.75, aes(x = V1, y = V2, color =SamplingSeason,shape=type,size=SamplingSeason))+ 
...
p
#ggsave("rbcl_microbiome_Procrustes2023.8.30.pdf", p, width = 5, height = 4)


# save.image(file = "Procrustes_rbcl.RData")
